...
module.exports = (async () => {
  try {
    ...
    const [rows] = await pool.execute('SHOW ');
    if (rows[0].count === 0) {
      await pool.execute(`
    CREATE TABLE Users (
    ...
    // Create other tables if users table doesn't exist
    await pool.execute(`
      CREATE TABLE Dogs (
  ...
  ((SELECT dog_id FROM Dogs WHERE name = 'Woggy'), '2025-06-22 09:00:00', 30, 'Burnside', 'completed'),
  ((SELECT dog_id FROM Dogs WHERE name = 'Stinky'), '2025-06-21 12:35:00', 25, 'Glenelg', 'completed');
      `);
